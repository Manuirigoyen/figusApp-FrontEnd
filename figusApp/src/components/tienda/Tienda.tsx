...
export const Tienda: React.FC = () => {
  ...
  const limpiarNombreProducto = (name: string): string =>
    name.replace(/\s*\(álbum\s*\d+\)/i, "").trim();
  ...
  return (
    <main id="mainContent" className="main-wrapper store-page position-relative text-white">
      ...
      <aside className={`store-cart-sidebar d-flex flex-column ${sidebarOpen ? "is-open" : ""}`}>
              ...
              <div className="d-flex flex-column gap-3">
                {carrito.map((item) => (
                  <div
                    ...
                    className="d-flex gap-2 align-items-center pb-2 border-bottom border-secondary"
                  >
                    <img
                      src={item.producto.cover_image}
                      alt={limpiarNombreProducto(item.producto.name)}
                      className="p-1 rounded bg-dark cart-item-img"
                      onError={(e) => (e.currentTarget.src = "/assets/img/default-product.png")}
                    />
                    <div className="flex-grow-1 min-w-0">
                      <h6 className="mb-0 text-truncate fs-6 fw-bold text-white">
                        {limpiarNombreProducto(item.producto.name)}
                      </h6>
                      <span className="text-white-50 small">
                        {item.cantidad} x ARS {calcularPrecioFinal(item.producto).toFixed(2)}
                      </span>
                    </div>
                    <button
                      type="button"
        ...
        <div className="cart-bottom-section pt-3 border-top border-secondary mt-auto">
          ...
          <div className="d-flex flex-column gap-2">
            <button
              type="button"
              onClick={() => (user ? navigate(ROUTES.PAGO) : navigate(ROUTES.LOGIN))}
              disabled={carrito.length === 0}
              className="btn-store-agregar w-100 fw-bold py-2"
            >
              Finalizar Compra
            </button>
            <button
      ...
      <section className="flex-grow-1 p-4 w-100">
        <div className="store-ad-banner mb-5 text-center">
          <a
            href="https://www.transfermarkt.com.ar/tailandia/startseite/verein/5676"
            target="_blank"
            rel="noopener noreferrer"
          >
            <img src={packBanner} alt="Promoción" className="img-fluid" />
          </a>
        </div>
        ...
        <div className="row align-items-center justify-content-center g-3 p-3 mb-5 store-filter-bar mx-0">
          ...
          <div className="col-12 col-sm-auto">
            <select
              ...
              onChange={(e) =>
                handleSetFiltro("ordenPrecio", e.target.value as typeof OPCIONES_ORDEN[number]["value"])
              }
            >
          ...
          <div className="col-12 col-sm-auto">
            <select
              className="form-select"
              value={filtros.filtroTipoCompra}
              onChange={(e) =>
                handleSetFiltro("filtroTipoCompra", e.target.value as typeof OPCIONES_CATEGORIA[number]["value"])
              }
            >
          ...
          <div className="col-auto">
            <div className="form-check form-switch d-flex align-items-center">
              <input
                ...
                type="checkbox"
                checked={filtros.filtroDescuento}
                onChange={(e) => handleSetFiltro("filtroDescuento", e.target.checked)}
              />
              ...
              <div className="store-product-card w-100">
                <div className="store-card-img-wrapper">
                  <img
                    ...
                    alt={limpiarNombreProducto(producto.name)}
                    className="product-card-img"
                  />
                </div>
                <div className="store-card-body">
                  <h5 className="store-product-title fs-6">
                    {limpiarNombreProducto(producto.name)}
                  </h5>
                  <div className="mb-2">
                    {verificarDescuentoActivo(producto) ? (
                      <div className="d-flex flex-column">
                        <span className="text-danger text-decoration-line-through fw-medium">
                          ARS {producto.price_usd.toFixed(2)}
                        </span>
                        <span className="text-success fw-bold">
                          ARS {calcularPrecioFinal(producto).toFixed(2)}
                        </span>
                      </div>
                    ) : (
                  ...
                  <div className="mt-auto">
                    <button
                      type="button"
                      onClick={() =>
                        navigate(ROUTES.PRODUCTO.replace(":id", String(producto.id)))
                      }
                      className="btn-store-comprar w-100 mb-2"
                    >
                      Comprar
                    </button>
                    <button
                      ...
                      className="btn-store-agregar w-100"
                    >
                      Agregar al carrito
                    </button>
                  </div>
