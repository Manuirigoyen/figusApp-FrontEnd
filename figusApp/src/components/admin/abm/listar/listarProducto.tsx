...
export const ListarProducto = () => {
  const [limiteProductos, setLimiteProductos] =
    useState<number>(5);

  const [productoBuscado, setProductoBuscado] =
    useState<Product | null>(null);

  const [productosListados, setProductosListados] =
    useState<Product[]>([]);
  ...
  const handleListarProductos = async (
    ...
    setLoading(true);
    setError('');
    setProductosListados([]);

    try {
      const data = await getProducts();

      setProductosListados(
        data.slice(0, limiteProductos),
      );
    } catch (err) {
      setError(
        err instanceof Error
      ...
      <div className="abm-list-wrapper">
        <section className="abm-list-section">
          <div className="abm-list-header">
            ...
            <p className="register-subtitle abm-subtitle mb-0">
              Busca un producto específico mediante su
              identificador.
            </p>
          </div>
        ...
        <section className="abm-list-section">
          <div className="abm-list-header">
            ...
            <p className="register-subtitle abm-subtitle mb-0">
              Lista varios productos mediante un límite de
              resultados.
            </p>
          </div>

          <form
            onSubmit={handleListarProductos}
            className="abm-form"
          >
            <div className="row g-3">
              <div className="col-12 abm-field">
                <label
                  htmlFor="inputLimiteProductos"
                  className="form-label"
                >
                  Límite de productos:{' '}
